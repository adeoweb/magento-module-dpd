  ...
  stages {
    stage('Build') {
      steps {
        ...
        sh "composer config http-basic.dev.adeoweb.biz:8453 $JENKINS_USER_USR $JENKINS_USER_PSW"
    ...
    stage('Test unit') {
      steps {
        sh 'mkdir results'
        sh './vendor/bin/phpcs --report=checkstyle --report-file=results/checkstyle.xml || true'
